...
use App\Http\Extends\ExtendedQueriesRelationships as QueriesRelationships;
...
use  App\Http\Extends\QueryBuilder as QueryBuilder;
 ...
 */
class ExtendedBuilder extends \Illuminate\Database\Eloquent\Builder implements BuilderContract
{
    ...
    {
        if ($id instanceof ExtendedModel) {
            $id = $id->getKey();
        }
    ...
    public function hydrate(array $items)
    {
        ...
        return $instance->newCollection(array_map(function ($item) use ($items, $instance) {
            ...
            if (count($items) > 1) {
                $model->preventsLazyLoading = ExtendedModel::preventsLazyLoading();
            }
    ...
    public function create(array $attributes = [])
    {
        ...
        $tape = tap($this->newModelInstance($attributes), function ($instance) {
            $instance->save();
        });
    ...
    protected function callScope(callable $scope, array $parameters = [])
    {
        ...
        if (count((array) $query->wheres) > $originalWhereCount) {
            $this->ExtendedaddNewWheresWithinGroup($query, $originalWhereCount);
        }
    ...
    protected function ExtendedaddNewWheresWithinGroup(QueryBuilder $query, $originalWhereCount)
    {
        ...
        $this->ExtendedgroupWhereSliceForScope(
            $query,
     ...
     * @return void
     */
    protected function ExtendedgroupWhereSliceForScope(QueryBuilder $query, $whereSlice)
    {
    ...
    public function newModelInstance($attributes = [])
    {
        ...
        $connexionName = $this->query->getConnection()->getName();
        $testerr = $this->model->newInstance($attributes)->setConnection($connexionName);
