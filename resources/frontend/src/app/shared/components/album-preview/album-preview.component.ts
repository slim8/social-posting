import { FileService } from './../../../features/my-album/services/file.service';
...
export class AlbumPreviewComponent implements OnInit {
  ...
  constructor(private iconService: NzIconService , private fileService: FileService ,) {
  ...
  selectImage(item : image){
    if(this.selectedImages.includes(item)){
      this.selectedImages = this.selectedImages.filter((img) => img != item);
    }else{
